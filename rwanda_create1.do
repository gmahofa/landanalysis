...
use "$lisd\Rwanda_Listing_2018", clear
...
save "$proc\Rwandalisting", replace

use "$lisd\hh_rost", clear /*from listing data*/
keep if s1q3 == 1
keep interview__id s1q2  
merge 1:1 interview__id using "$proc\Rwandalisting"
keep if _merge == 3
drop _merge
encode interview__id, gen(hhid)
drop interview__id
drop if s1q2 == .a
save "$proc\hhdlisting", replace
********************************************************************************
********************************************************************************
*LOADING THE HOUSEHOLD ROSTER FROM HOUSEHOLD FULL DATA

use "$hhdta\Rwanda_household_2018", clear
d /*describe the data*/
...
recode d_one (2/3 = 1), g(dall)
recode dall 1=0 if dincorrt==1
lab var dall "No incorrect answer"
g dnone=dincorr==1 & d1==0
...
recode d_onesles (2/3 = 1), g(dall_sles)
recode dall_sles 1=0 if dincorrt_sles==1
lab var dall_sles "No incorrect answer"
g dnone_sles=dincorrt_sles==1 & d1_sles==0
...
recode d_oneuse (2/3 = 1), g(dall_use)
recode dall_use 1=0 if dincorrt_use==1
lab var dall_use "No incorrect answer"
g dnone_use=dincorrt_use==1 & d1_use==0
...
g dresid = s2_landuse__1 == 1 &s2_landuse__2 == 0&s2_landuse__3 == 0 | s2_landuse__4 == 1 & ///
s2_landuse__5 == 0&s2_landuse__6 == 0 if s2_landuse__1!=.&s2_landuse__2!=.&s2_landuse__3!=.&s2_landuse__4!=.& ///
s2_landuse__5!=.&s2_landuse__6!=.

g dagric = s2_landuse__1 == 0&s2_landuse__2 == 1 | s2_landuse__3 == 1&s2_landuse__4 == 0 | ///
s2_landuse__5 == 1 | s2_landuse__6 == 1 if s2_landuse__1!=.&s2_landuse__2!=.&s2_landuse__3!=.&s2_landuse__4!=.& ///
s2_landuse__5!=.&s2_landuse__6!=.

g mixeduse = dresid == 1&dagric == 1
*/
...
save "$proc\acquired2018", replace
restore

preserve
keep if s2q3 == 1
keep if s2q1 <= 2012
keep interview__id oproster__id area 
save "$proc\oldown2013", replace
restore

preserve
keep interview__id oproster__id upi area s2q3 s2q6 nosubdiv map_yes ongoing ///
noobtain 
keep if s2q3 == 3
save "$proc\give2013", replace
restore

preserve
keep interview__id upi oproster__id s2q3 area s2q6 s2q10
keep if s2q3==2 | s2q3==3
sort interview__id oproster__id
save "$proc\fraggive2013", replace
restore

drop interview__key oproster__id village_parcel villparcel owner_parcel upi ///
parcel_name
save "$proc\oldparcel1", replace
...
preserve
sort interview__id oproster__id
merge interview__id oproster__id using "$proc\fraggive2013"
drop if _mer==1
...
keep if s2q11f >=2013&s2q11f<=2018
drop s2q11f
save "$proc\fraggive", replace
restore


save "$proc\fragroster_1.dta", replace
keep if s2q11f >=2013&s2q11f<=2018
keep interview__id oldfragid female_op male_opfr parent son_daughter other_tofra frag_sold frag_beq
save "$proc\fragroster_1to.dta", replace 
use "$tmp\household2018_to", clear
keep interview__id province_cd 
merge 1:m interview__id using "$proc\fragroster_1to.dta"
keep if _merge == 3
drop interview__id _merge
save "$proc\fragroster_an", replace
/*cap table1 province_cd using $tmp\, id(oldfragid) order
export excel using "$temp\oldfrag_d2", firstrow(var) replace
*/

use "$proc\fragroster_an", clear
keep if frag_sold == 1
cap table1 province_cd using $tmp\, id(oldfragid) order
export excel using "$proc\oldfrag_mkt", firstrow(var) replace
use "$proc\fragroster_an", clear
/*keep if frag_beq == 1
cap table1 province_cd using $tmp\, id(oldfragid) order
...
/*
g dresid = s3_landuse__1 == 1&s3_landuse__2 == 0&s3_landuse__3 == 0 | s3_landuse__4 == 1& ///
s3_landuse__5 == 0&s3_landuse__6 == 0 if s3_landuse__1!=.&s3_landuse__2!=.&s3_landuse__3!=.&s3_landuse__4!=.& ///
s3_landuse__5!=.&s3_landuse__6!=.

g dagric = s3_landuse__1 == 0&s3_landuse__2 == 1 | s3_landuse__3 == 1&s3_landuse__4 == 0 | ///
s3_landuse__5 == 1 | s3_landuse__6 == 1 if s3_landuse__1!=.&s3_landuse__2!=.&s3_landuse__3!=.&s3_landuse__4!=.& ///
s3_landuse__5!=.&s3_landuse__6!=.
...
save "$proc\acquired2018to", replace

use "$tmp\newparcelto", clear
ren area_acq area
append using "$proc\acquired2018to"
save "$tmp\newpacquired", replace
...
so interview__id hhroster__id
merge m:m interview__id hhroster__id using "$hhdta\hhroster"
keep if _merge == 3
drop _merge
...
*/
so upi
save "$proc\acqname", replace
use "$hhdta\UPI_Gender_dup", clear
drop interview__id area village_parcel oproster__id
so upi
save "$proc\upi_gender", replace
use "$proc\acqname", clear
mer 1:m upi using "$proc\upi_gender"
keep if _mer == 3
drop _mer 
duplicates drop 
...
so newparcelid
save "$proc\acqname_upigendr", replace
append using "$proc\regname"
so interview__id
merge m:m interview__id using "$proc\newparcelmgd"
drop if _mer == 1
drop _mer
keep interview__id maleonly-joint interview__id newproster__id wtp_value_newr-dcdisp s3q10 s3q12 ///
area-registered
egen newparcelid = group(interview__id newproster__id)
...
use "$proc\newparcelmgd", clear /*constructing market status variable to be merged with hhd data*/
...
inspect s4q10
sum s4q10
save "$proc\toroster1", replace
keep interview__id toroster__id s4q4 soldparcid area_sold cadmap_out ongproc_out notobt_out ///
subdivided_out s4q7 
ren s4q4 upi
so interview__id toroster__id
mer m:m interview__id toroster__id using "$hhdta\tofragroster"
keep if _mer == 3
drop _mer interview__key tofragroster__id s4q11a-s4q11e s4q11h
save "$proc\toroster1_to", replace
***
use "$proc\give2013", clear
so interview__id oproster__id
mer m:m interview__id oproster__id using "$hhdta\fragroster"
drop if _mer == 1
drop _mer
keep s2q11g upi s2q11f  interview__id oproster__id s2q6 nosubdiv area map_yes ongoing noobtain
ren (oproster__id s2q6 nosubdiv map_yes ongoing noobtain area s2q11g s2q11f) (toroster__id ///
subdivided_out s4q7 cadmap_out ongproc_out notobt_out area_sold s4q11g s4q11f)
save "$proc\give2013to", replace
use "$proc\toroster1_to", clear
append using "$proc\give2013to"
drop soldparcid
egen soldparcid = group(interview__id toroster__id)
save "$proc\give2013tom", replace
use "$tmp\household2018_to", clear
keep interview__id province_cd village_cd
merge 1:m interview__id using "$proc\give2013tom"
keep if _merge == 3
drop _merge
...
frag_sold frag_beq)
append using "$proc\fraggive"
save "$proc\trn_out", replace
restore

save "$proc\tofragroster1.dta", replace
keep if s4q11f>=2013&s4q11f<=2018
keep interview__id tofragid female_frag_acq male_frag_acq parent son_daughter other_tofra tofrag_sold tofrag_beq
save "$proc\tofragroster1_to", replace
use "$tmp\household2018_to", clear
keep interview__id province_cd 
merge 1:m interview__id using "$proc\tofragroster1_to"
keep if _merge == 3
drop interview__id _merge
save "$proc\tofragroster1_mgd", replace
cap table1 province_cd using $tmp\, id(tofragid) order
export excel using "$proc\tofrag_inf", firstrow(var) replace
use "$proc\tofragroster1_mgd", clear
keep if tofrag_sold == 1
cap table1 province_cd using $tmp\, id(tofragid) order
export excel using "$proc\tofragid_mkt", firstrow(var) replace
use "$proc\tofragroster1_mgd", clear
keep if tofrag_beq == 1
cap table1 province_cd using $tmp\, id(tofragid) order
export excel using "$proc\tofragid_inh", firstrow(var) replace

use "$tmp\household2018_to", clear
keep interview__id province_cd 
save "$proc\hhdprovince", replace
use "$proc\trn_out", clear
so interview__id
merge m:1 interview__id using "$proc\hhdprovince"
keep if _mer == 3
drop _mer
merge m:1 interview__id using "$tmp\hhdgender"
keep if _mer == 3
...
keep if _mer == 3
drop _mer
mer m:1 interview__id using "$proc\hhdregion"
keep if _mer == 3
...
inspect s6q5
preserve
keep asset interview__id assets__id asset s6q3 assetbuy s6q5
save "$proc\assetindex", replace
restore
...
so interview__id
merge 1:1 interview__id using "$proc\assets_agric"
drop _merge
save "$proc\asset_quartiles", replace
keep interview__id wealth_qurtile
save "$proc\assetqua_tm", replace
collapse (sum) parcel_out, by(village_cd)
so village_cd
save "$proc\village_out", replace
*/
*******
